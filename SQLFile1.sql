...
INSERT INTO ClientesRestritos (idCliente) VALUES (5);							

SELECT * FROM ClientesRestritos;

----------------------------------------------FORNECEDORES RESTRITOS-----------------------------------------------

INSERT INTO FornecedoresRestritos (idFornecedor) VALUES (3);			
...
--        <<   Informações completas de Medicamentos   >>
SELECT m.CDB, m.Nome AS "Medicamento", m.ValorVenda, cat.Categoria, s.Situacao AS "Status Medicamentos", m.DataCadastro
FROM Medicamentos m
JOIN CategoriasMed cat ON cat.id = m.Categoria
JOIN SituacaoMed s ON s.id = m.Situacao
ORDER BY m.Nome;

--        <<   Informações completas de Produções   >>
SELECT p.idProducao, p.DataProducao, m.Nome AS "Medicamento", p.Quantidade AS "Qtd Medicamento",
		pa.Nome AS "Principio Ativo", itp.Quantidade AS "Qtd Principio"
FROM Producoes p
JOIN Medicamentos m ON p.CDB = m.CDB
JOIN ItensProducoes itp ON p.idProducao = itp.idProducao
JOIN PrincipiosAtivo pa ON pa.idPrincipioAt = itp.idPrincipioAt
ORDER BY p.DataProducao;

--        <<   Informações completas de Compras   >>
SELECT c.idCompra, f.RazaoSocial AS Fornecedor, c.DataCompra, pa.Nome AS "Principio Ativo",
		ic.Quantidade, ic.ValorUnitario, ic.TotalItem, c.ValorTotal
FROM Compras c
JOIN Fornecedores f ON f.idFornecedor = c.idFornecedor
JOIN ItensCompras ic ON ic.idCompra = c.idCompra
JOIN PrincipiosAtivo pa ON pa.idPrincipioAt = ic.idPrincipioAt
ORDER BY c.idCompra;
